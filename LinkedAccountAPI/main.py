...
from .models import LinkedAccountSubmission, SubmissionResponse, SubmissionSummary
...
app = FastAPI(
    title="WLTH Linked Account Nomination API",
    version="1.0.0",
    description=(
        "Digitised submission API for the WLTH Linked Account Nomination Form. "
...
app.mount("/static", StaticFiles(directory=str(BASE_DIR / "static")), name="linked-account-static")
...
@app.post(
    ...
    summary="Submit a linked account nomination form",
    status_code=201,
)
def submit_form(body: LinkedAccountSubmission):
    ...
    try:
        ...
        loan_no   = body.loan_account_number
...
def download_pdf(submission_id: str):
    ...
    filename = f"wlth_linked_account_{submission_id[:8]}.pdf"
    return Response(
        content=pdf_bytes,
...
@app.get(
    "/api/v1/submissions/{submission_id}/bank-statements/{account_idx}",
    summary="Download the bank statement for a linked account",
)
def download_bank_statement(submission_id: str, account_idx: int):
    sub = get_submission(submission_id)
    if not sub:
        raise HTTPException(status_code=404, detail="Submission not found")

    accounts = sub["data"].get("linked_accounts", [])
    if account_idx < 0 or account_idx >= len(accounts):
        raise HTTPException(status_code=404, detail="Account index out of range")

    account = accounts[account_idx]
    b64 = account.get("bank_statement_base64", "")
    filename = account.get("bank_statement_filename", "bank_statement")

    if not b64:
        raise HTTPException(status_code=404, detail="No bank statement on file")
