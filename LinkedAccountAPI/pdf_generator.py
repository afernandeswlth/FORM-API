...
def _account_block(idx: int, account: dict, s: dict):
    """Build a single linked-account section."""
    label = f"Linked Account {idx}" if idx > 0 else "Linked Account"
    rows = []
    rows.append(Paragraph(label, s["section"]))
...
def _authorisation_block(idx: int, borrower: dict, auth: dict, s: dict):
    ...
    suffix = f" ({name})" if len else ""

    elems = []
    elems.append(Paragraph(f"Customer Authorisation{suffix}", s["section"]))
...
def _page_footer(canvas, doc):
    ...
    txt = (
        "WLTH-V1.1  |  Linked Account Nomination Form  |  "
...
def generate_pdf(data: dict) -> bytes:
    ...
    form_type = data.get("form_type", "single")
    if form_type == "single":
        story.append(Paragraph("New Linked Account Nomination", s["title"]))
        story.append(Paragraph("Set up a single new linked account to your loan", s["subtitle"]))
    else:
        story.append(Paragraph("New Linked Account Nomination", s["title"]))
        story.append(Paragraph("Set up multiple linked accounts to your loan", s["subtitle"]))

    # ── INTRO TEXT ────────────────────────────────────────────────────────
    story.append(Paragraph(
        "I/We authorise and request you setup my/our account(s) described below and in the future, "
        "may be direct debited by me/us in connection with my/our mortgage loan and which may be "
        "debited or charged through the Bulk Electronic Clearing System (BECS) to my/our account "
        "conducted with WLTH. <b>Direct Debit ID No. 460095</b>",
        s["body"],
    ))
    ...
    # ── LINKED ACCOUNTS ───────────────────────────────────────────────────
    story.append(Paragraph("Nominated Account(s) to be Linked to Loan", s["section"]))
    linked_accounts = data.get("linked_accounts", [])

    if form_type == "single" or len(linked_accounts) <= 2:
        for i, acc in enumerate(linked_accounts):
            label = "Linked Account" if len(linked_accounts) == 1 else f"Linked Account {i + 1}"
            story.append(Paragraph(label, s["label"]))
            story.append(_field_table([
                ("Financial Institution", acc["financial_institution"]),
                ("Branch",               acc["branch"]),
                ("Account Holders / Title", acc["account_holders"]),
                ("BSB No.",              acc["bsb_number"]),
                ("Account No.",          acc["account_number"]),
            ], col_widths=[55 * mm, 105 * mm]))
            story.append(Spacer(1, 3 * mm))
    else:
        # Render accounts in a 2-column grid for 3-4 accounts
        pairs = [linked_accounts[i:i + 2] for i in range(0, len(linked_accounts), 2)]
        for pair_i, pair in enumerate(pairs):
            grid_cells = []
            for j, acc in enumerate(pair):
                idx = pair_i * 2 + j + 1
                cell_content = [
                    Paragraph(f"Linked Account {idx}", s["label"]),
                    _field_table([
                        ("Financial Institution", acc["financial_institution"]),
                        ("Branch",               acc["branch"]),
                        ("Account Holders",       acc["account_holders"]),
                        ("BSB No.",              acc["bsb_number"]),
                        ("Account No.",          acc["account_number"]),
                    ], col_widths=[30 * mm, 50 * mm]),
                ]
            ...
            grid = Table([[grid_cells[0], grid_cells[1]]], colWidths=[82 * mm, 82 * mm])
    ...
    story.append(Paragraph(
        "I/we, request WLTH to add the above account information to my/our WLTH Internet Portal "
        "and acknowledge this account may be used for Direct Debits in the future. "
        "(NB: Direct debiting is not available on the full range of accounts. If in doubt please "
        "refer to your Financial Institution). I/we acknowledge that this Direct Debit arrangement "
        "is governed by the terms of the Client Service Agreement overleaf.",
        s["body"],
    ))
    story.append(Spacer(1, 2 * mm))
...
def _build_terms_page(story: list, s: dict) -> None:
    ...
    story.append(Spacer(1, 2 * mm))
    story.append(HRFlowable(width="100%", thickness=1, color=WLTH_DARK, spaceAfter=3 * mm))
    story.append(Paragraph("Linked Account Nomination", s["terms_h1"]))
