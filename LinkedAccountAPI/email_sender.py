...
def _email_body(data: dict, submission_id: str) -> str:
    ...
    return (
        f"A new Linked Account Nomination has been submitted.\n"
        f"\n"
        ...
        f"  • Bank statement(s) for each nominated account\n"
        f"\n"
        f"--\n"
        f"WLTH Linked Account Nomination System\n"
    )
...
def send_submission_email(
    ...
    base_subject = f"Linked Account Nomination – Loan {loan_no}"
    ...
    # Attach each bank statement
    accounts = data.get("linked_accounts", [])
    for idx, account in enumerate(accounts):
        b64 = account.get("bank_statement_base64", "")
        filename = account.get("bank_statement_filename", f"bank_statement_{idx + 1}")
        if not b64:
            continue
        raw_b64 = b64.split(",", 1)[1] if "," in b64 else b64
        header = b64.split(",", 1)[0] if "," in b64 else ""
        mime_type = (
        ...
        part.set_payload(base64.b64decode(raw_b64))
        encode_base64(part)
