...
def _repayment_label(ls: dict) -> str:
    ...
    label = "Principal & Interest" if rt == "principal_and_interest" else "Interest Only"
    if rt == "interest_only" and ls.get("interest_only_years"):
...
def _email_body(data: dict, submission_id: str) -> str:
    ...
    switch_lines = "\n".join(
        f"  • Loan {ls['loan_account_number']}  |  {_repayment_label(ls)}  |  {_rate_label(ls)}"
        for ls in switches
    )
    submitted_at = datetime.now(timezone.utc).strftime("%d %b %Y at %H:%M UTC")
    reason = data.get("reason_for_request") or "—"

    return (
        f"A new Product Switch Request has been submitted.\n"
        f"\n"
        ...
        f"\n"
        f"--\n"
        f"WLTH Product Switch Request System\n"
    )
...
def send_submission_email(
    ...
    borrowers = data.get("borrowers", [])
    names = ", ".join(f"{b['given_names']} {b['surname']}" for b in borrowers)
    base_subject = f"Product Switch Request – {names}"
