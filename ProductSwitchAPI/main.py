...
from .models import ProductSwitchSubmission, SubmissionResponse, SubmissionSummary
...
app = FastAPI(
    title="WLTH Product Switch Request API",
    version="1.0.0",
    description=(
        "Digitised submission API for the WLTH Product Switch Request Form. "
...
app.mount("/static", StaticFiles(directory=str(BASE_DIR / "static")), name="product-switch-static")
...
@app.post(
    ...
    summary="Submit a Product Switch Request form",
    status_code=201,
)
def submit_form(body: ProductSwitchSubmission):
    ...
    try:
        ...
        loan_no   = body.loan_switches[0].loan_account_number
...
def download_pdf(submission_id: str):
    ...
    filename = f"wlth_product_switch_{submission_id[:8]}.pdf"
    return Response(
        content=pdf_bytes,
