...
from .models import DirectDebitSubmission, SubmissionResponse, SubmissionSummary
...
app = FastAPI(
    title="WLTH Direct Debit Request API",
    version="1.0.0",
    description=(
        "Digitised submission API for the WLTH Direct Debit Request Form. "
...
app.mount("/static", StaticFiles(directory=str(BASE_DIR / "static")), name="direct-debit-static")
...
@app.post(
    ...
    summary="Submit a Direct Debit Request form",
    status_code=201,
)
def submit_form(body: DirectDebitSubmission):
...
@app.get(
    ...
    summary="Download the bank statement for a debit account",
)
def download_bank_statement(submission_id: str, account_idx: int):
    ...
    accounts = sub["data"].get("debit_accounts", [])
