...
from .models import OpenOffsetSubmission, SubmissionResponse, SubmissionSummary
...
app = FastAPI(
    title="WLTH Open Offset Account API",
    version="1.0.0",
    description=(
        "Digitised submission API for the WLTH Open an Offset Account Form. "
...
app.mount("/static", StaticFiles(directory=str(BASE_DIR / "static")), name="open-offset-static")
...
@app.post(
    ...
    summary="Submit an Open Offset Account form",
    status_code=201,
)
def submit_form(body: OpenOffsetSubmission):
    ...
    try:
        ...
        account_no = body.linked_account_number
        reply_to  = get_thread_message_id(account_no)
        msg_id    = send_submission_email(submission_id, sub["data"], pdf_bytes, reply_to)
        # Only save the message ID for the very first submission on this account
        if not reply_to:
            save_thread_message_id(account_no, msg_id)
